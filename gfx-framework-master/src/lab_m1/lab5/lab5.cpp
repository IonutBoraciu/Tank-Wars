...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    glm::mat4 modelMatrix = glm::mat4(1);
    modelMatrix *= glm::translate(modelMatrix, glm::vec3(2.5f, 2.5f, 0));
    modelMatrix *= glm::rotate(modelMatrix, RADIANS(45.0f), glm::vec3(0, 1, 0));
    RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);

    modelMatrix = glm::mat4(1);
    modelMatrix *= glm::translate(modelMatrix, glm::vec3(2.5f, 0, 3.5f));
    modelMatrix *= glm::rotate(modelMatrix, RADIANS(-45.0f), glm::vec3(0, 1, 0));
    modelMatrix *= glm::scale(modelMatrix, glm::vec3(0.5, 2.0, 1.15));
    RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_K)) {
            Left -= 16.0f * deltaTime;
            Right += 16.0f * deltaTime;
            Bottom -= 9.0f * deltaTime;
            Top += 9.0f * deltaTime;
